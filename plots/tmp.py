...
cache = dir_archive('/home/users/hbronson/tmp/tmp2/CMSSW_10_2_9/src/tW_scattering_dilep/caches/forward_2017_OS', serialized=True)
cache.load()
...
cfg = loadConfig()
plot_dir = os.path.expandvars(cfg['meta']['plots']) + 'test' + '/UL/' + 'test' + 'dilep_2018'
version_dir = '/OS_2017/'
...
makePlot(output, 'lead_lep', 'eta',
        data=data,
        bins=None, log=False, normalize=TFnormalize, axis_label=r'$\eta\ lead \ lep$',
        new_colors=my_colors, new_labels=my_labels,
        order=order,
        omit=omit,
        signals=signals,
