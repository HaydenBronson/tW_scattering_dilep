...
class exampleProcessor(processor.ProcessorABC):
    ...
    def __init__(self):
        ...
        eta_axis            = hist.Bin("eta",       r"$\eta$", 60, 0,11)
        delta_eta_axis      = hist.Bin('deltaeta',  r"DeltaEta", 60, 0, 11)
    ...
    def process(self, df):
        ...
        output['fw_max_deltaeta'].fill(dataset=dataset, eta=deltaEta_jj[event_selection].flatten(), weight=df['weight'][event_selection]*cfg['lumi'])
        output['lj_max_deltaeta'].fill(dataset=dataset, eta=deltaEta_lj[event_selection].flatten(), weight=df['weight'][event_selection]*cfg['lumi'])
...
def main():
    ...
    # histograms
    histograms = []
    histograms += ['N_ele', 'N_mu', 'N_diele', 'N_dimu', 'MET_pt', 'pt_spec_max', 'MT', 'HT', 'ST', 'mbj_max', 'mjj_max', 'mlb_max', 'mlb_min', 'mlj_max', 'mlj_min', 'N_b', 'N_jet', 'N_spec', 'fw_max_deltaeta', 'lj_max_deltaeta']
...
output = cache.get('simple_output')
for select in ['dilep','lepveto','SS','njet4','njet5','central3','nbtag','Mll','MET>50']:
    SB = output['tW_scattering'][select]/(output['ttbar'][select]+output['diboson'][select]+output['TTW'][select]+output['TTX'][select]+output['DY'][select])
    print(select,SB)