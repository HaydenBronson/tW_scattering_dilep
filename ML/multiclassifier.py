...
def test_train(test, train, y_test, y_train, labels=[], bins=25, node=0, plot_dir=None, weight_test=None, weight_train=None):
    ...
    for i, label in enumerate(labels):
        ...
        h[label+'_test'].plot(color=colors[i], histtype="step", ls='--', linewidth=3)
        h[label+'_train'].plot(color=colors[i], histtype="step", linewidth=3)
...
if __name__ == '__main__':
    ...
    variables = [
       ...
       'lead_lep_energy', 'lead_lep_px', 'lead_lep_py', 'lead_lep_pz', 
       'sublead_lep_energy', 'sublead_lep_px', 'sublead_lep_py', 'sublead_lep_pz',
       'j1_energy', 'j1_px', 'j1_py', 'j1_pz',
       'j2_energy', 'j2_px', 'j2_py', 'j2_pz',
       'j3_energy', 'j3_px', 'j3_py', 'j3_pz',
       'j4_energy', 'j4_px', 'j4_py', 'j4_pz',
       'j5_energy', 'j5_px', 'j5_py', 'j5_pz',
       'weight', 'score_topW', 'score_ttW', 'score_ttZ', 'score_ttH',
       'score_ttbar', 'score_best',
    ]
    ...
    """
    if not args.load:
        df_train, df_test, y_train_int, y_test_int = prepare_data('/hadoop/cms/store/user/dspitzba/ML/multiclass_input_v4.h5', preselection, reuse=False, fout='data/multiclass_input_v4_split_%s.h5'%args.version, label_ID='label_cat' if args.cat else 'label')
    else:
        df_train, df_test, y_train_int, y_test_int  = prepare_data('data/multiclass_input_v4_split_%s.h5'%args.version, preselection, reuse=True, label_ID='label_cat' if args.cat else 'label')
    """
    df_train, df_test, y_train_int, y_test_int = prepare_data('/hadoop/cms/store/user/dspitzba/ML/multiclass_input_v4.h5', preselection, reuse=False, fout='data/multiclass_input_v4_split_%s.h5'%args.version, label_ID='label_cat' if args.cat else 'label')
    ...
    print ("w/o LBN, baseline expectations are:")
    ...
    print ("w/ LBN, baseline expectations are:")
    ...
    makePlot(output, 'score_topW_pos', 'score',
         ...
         order=['rare', 'TTH', 'ttbar', 'TTZ', 'TTW', 'topW_v4'],
         lumi=137,
        )
    ...
    makePlot(output, 'score_topW_pos', 'score',
         ...
         new_colors=my_colors, new_labels=my_labels,
         order=['rare', 'TTH', 'TTZ', 'TTW', 'topW_v4'],
         omit=['ttbar', 'rare'],
