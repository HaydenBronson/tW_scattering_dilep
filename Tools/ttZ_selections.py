...
class Selection:
    ...
    def trilep_baseline(self, omit=[], cutflow=None, tight=False):
        ...
        diele   = choose(self.ele, 2)
        dilep   = cross(self.mu, self.ele)
        divemu    = choose(self.mu_veto, 2)
        diveele   = choose(self.ele_veto, 2)

        OS_dimu = dimu[(dimu['0'].charge*dimu['1'].charge < 0)]
        OS_diele = diele[(diele['0'].charge*diele['1'].charge < 0)]
        OS_divemu = divemu[(divemu['0'].charge*divemu['1'].charge < 0)]
        OS_diveele = diveele[(diveele['0'].charge*diveele['1'].charge < 0)]
        ...
        SFOSveto = ((ak.num(OS_diele) + ak.num(OS_dimu)) ==0) & ((ak.num(OS_diveele) +ak.num(OS_divemu)) ==0)
        dilepton = choose(lepton, 2)
        SS_dilep = (dilepton['0'].charge*dilepton['1'].charge > 0)
        ...
        self.selection.add('SSloose',       los_trilep_SS)
        self.selection.add('filter',        self.filters)
        self.selection.add('trigger',       triggers)
        self.selection.add('p_T(lep0)>25',  lep0pt)
        self.selection.add('p_T(lep1)>20',  lep1pt)
