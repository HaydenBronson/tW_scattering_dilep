...
class Selection:
    ...
    def dilep_baseline(self, omit=[], cutflow=None, tight=False, SS=True):
        ...
        #is_dilep   = ( ((ak.num(self.ele_veto) + ak.num(self.mu_veto))==2) )
        ...
        self.mu_veto['p4'] = get_four_vec_fromPtEtaPhiM(self.mu_veto, get_pt(self.mu_veto), self.mu_veto.eta, self.mu_veto.phi, self.mu_veto.mass, copy=False)
        self.ele_veto['p4'] = get_four_vec_fromPtEtaPhiM(self.ele_veto, get_pt(self.ele_veto), self.ele_veto.eta, self.ele_veto.phi, self.ele_veto.mass, copy=False)
        self.selection = PackedSelection()
        ...
        #is_dilep   = ( ((ak.num(self.ele_veto) + ak.num(self.mu_veto))==2) )
        is_dilep   = ( ((ak.num(self.ele) + ak.num(self.mu))==2) & ((ak.num(self.ele_veto) + ak.num(self.mu_veto))==2) )
        pos_charge = ((ak.sum(self.ele_veto.pdgId, axis=1) + ak.sum(self.mu_veto.pdgId, axis=1))<0)
        neg_charge = ((ak.sum(self.ele_veto.pdgId, axis=1) + ak.sum(self.mu_veto.pdgId, axis=1))>0)
