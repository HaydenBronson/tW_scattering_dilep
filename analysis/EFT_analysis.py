...
if __name__ == '__main__':
    ...
    for year in years:
        ...
        import numpy
        if scan == 'ctW':
            EFT_points = numpy.arange(-2.5,2.5,0.5)
        elif scan == 'ctp':
            EFT_points = numpy.arange(-2.5,2.5,0.5)
        yaxis = numpy.zeros(len(EFT_points)) 
        
        for i in range(len(EFT_points)):
            EFT_point = EFT_points[i]
            ...
            bsm_card_sr1 = makeCardFromHist(
                ...
                overflow='all',
                ext='_BSM',
                signal_hist=signal_hists['%s_SR_1'%year]['topW_full_EFT_%s_%s'%(scan, EFT_point)],
        ...
        bsm_card_sr1 = makeCardFromHist(
            ...
            overflow='all',
            ext='_BSM',
            signal_hist=signal_hists['%s_SR_1'%year]['topW_full_EFT_ctW_%s'%EFT_point],
            #systematics = get_systematics(output, 'node0_score_transform_pp', year, correlated=correlated), 
        )
